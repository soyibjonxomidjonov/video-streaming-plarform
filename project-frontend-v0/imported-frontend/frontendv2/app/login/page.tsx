...
import { api, ApiError, getGoogleAuthUrl, setToken, setRefreshToken } from '@/lib/api'
...
export default function LoginPage() {
  ...
  const verifyCode = async (e: FormEvent) => {
    ...
    try {
      ...
      // ✅ TUZATILDI: faqat `access` va `refresh` — boshqa yo'q
          ...
          {step === 'email' ? (
            <>
              ...
              <button
                ...
                className="mt-5 flex w-full items-center justify-center gap-3 rounded-xl border border-border py-3 text-sm font-semibold transition hover:bg-surface"
              >
                <span className="flex size-5 items-center justify-center rounded-full bg-white text-xs font-black" style={{ color: '#4285f4' }}>G</span>
                Google bilan kirish
              </button>
