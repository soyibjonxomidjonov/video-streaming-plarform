...
export default function WatchClient({ id, type, title, poster, streamUrl, description, rating, year, episodes, initialComments }: Props) {
  ...
  const saveProgress = useCallback(() => {
    ...
    if (!video || !video.duration || !targetId) return
    const task = type === 'movie' ? api.progressMovie(Number(id), video.currentTime, video.duration) : api.progressEpisode(targetId, video.currentTime, video.duration)
