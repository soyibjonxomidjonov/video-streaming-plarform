...
import { Heart, MessageCircle, Play, Send, Star, Tv } from 'lucide-react'
...
export default function SeriesDetailClient({ id, title, episodes, initialComments }: Props) {
  ...
  const toggleFavorite = async () => {
    ...
    try {
      if (favorite && favoriteId) {
        await api.unfavoriteSeries(favoriteId)
        setFavorite(false)
        setFavoriteId(null)
        showNotice('Sevimlilardan olib tashlandi')
      } else {
        const res = await api.favoriteSeries(Number(id))
        setFavorite(true)
  ...
  const submitRating = async (value: number) => {
    ...
    setRated(value)
    try {
      await api.rateSeries(Number(id), value)
  ...
  const postComment = async (e: FormEvent) => {
    ...
    setComment('')
    try {
      const created = await api.commentSeries(Number(id), text)
            ...
            {[1, 2, 3, 4, 5].map(val => (
              ...
              >
                <Star
                  size={22}
                  style={{
                    fill: val <= (hoverRating || rated) ? '#f5a623' : 'transparent',
                    color: val <= (hoverRating || rated) ? '#f5a623' : '#2a2a30',
                  }}
                />
              </button>
            ))}
          </div>
          {rated > 0 && <span className="text-sm font-semibold" style={{ color: '#f5a623' }}>({rated}/5)</span>}
        </div>

        {notice && <p className="ml-auto text-sm font-medium" style={{ color: '#22c55e' }}>{notice}</p>}
      </div>

      {/* Episodes list */}
      <section id="episodes-section">
        <h2 className="mb-4 flex items-center gap-2 font-display text-xl font-bold">
          <Tv size={20} style={{ color: '#38bdf8' }} />
          Qismlar / Epizodlar ({episodes.length})
        </h2>

        {episodes.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border py-12 text-center">
            <p className="text-sm text-muted-foreground">Ushbu serialda qismlar mavjud emas</p>
          </div>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {episodes.map((ep, idx) => (
              <Link
                ...
                className="group flex items-center gap-4 rounded-2xl p-4 transition hover:border-primary/50"
                style={{ background: '#16161a', border: '1px solid #2a2a30' }}
              >
                <div
                  className="flex size-12 shrink-0 items-center justify-center rounded-xl font-display text-lg font-bold text-black transition group-hover:scale-105"
                  style={{ background: 'linear-gradient(135deg, #f5a623, #b3720f)' }}
                >
                  {ep.episode_number ?? idx + 1}
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="truncate font-semibold text-sm group-hover:text-amber-400 transition">{mediaTitle(ep)}</h3>
                  <p className="mt-0.5 text-xs text-muted-foreground">Epizod {ep.episode_number ?? idx + 1}</p>
                </div>
                <div className="flex size-8 items-center justify-center rounded-full bg-surface text-muted-foreground transition group-hover:bg-amber-400 group-hover:text-black">
      ...
      <section id="comments-section">
        ...
        {comments.length === 0 ? (
          ...
          <ul className="flex flex-col gap-3">
            {comments.map(item => (
              <li key={item.id} className="rounded-xl p-4 animate-fade-in" style={{ background: '#16161a', border: '1px solid #2a2a30' }}>
                <div className="mb-1.5 flex items-center gap-2">
                  <div className="flex size-7 items-center justify-center rounded-full text-xs font-bold text-black" style={{ background: '#f5a623' }}>
