    ...
    <div>
      <div className="mb-5 flex flex-col gap-3">
        <div className="flex gap-2">
          <div className="flex flex-1 items-center gap-2 rounded-xl border border-border bg-surface px-4 py-3">
            ...
            <input
              ...
              style={{ border: 'none' }}
            />
            {search && <button onClick={() => handleSearch('')} className="text-muted-foreground hover:text-foreground"><X size={14} /></button>}
          </div>
          <button
        ...
        {showFilters && (
          <div className="animate-slide-up rounded-xl border border-border bg-surface p-4">
                ...
                {SORT_OPTIONS.map(opt => (
                  <button
                    ...
                    className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${
                      ordering === opt.value ? 'bg-primary text-primary-foreground' : 'bg-surface-2 text-muted-foreground hover:text-foreground'
                    }`}
                  >
                    {opt.label}
                  </button>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${
                      selectedGenre === null ? 'bg-primary text-primary-foreground' : 'bg-surface-2 text-muted-foreground hover:text-foreground'
                    }`}
                  >
                    Barchasi
                  </button>
                  {genres.map(g => (
                    <button
                      ...
                      className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${
                        selectedGenre === g.id ? 'bg-primary text-primary-foreground' : 'bg-surface-2 text-muted-foreground hover:text-foreground'
                      }`}
                    >
                      {g.name}
                    </button>
            ...
            {hasFilters && (
              <button onClick={clearFilters} className="mt-4 flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-foreground">
                <X size={12} /> Tozalash
              </button>
            )}
          </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border py-16 text-center">
          ...
          {hasFilters && <button onClick={clearFilters} className="text-sm font-semibold text-primary">Filtrlarni tozalash</button>}
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          ...
          {hasMore && (
            <div className="mt-8 flex justify-center">
              <button
                id="series-load-more"
                onClick={() => void fetchSeries(page + 1)}
