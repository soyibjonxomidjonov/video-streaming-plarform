        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[rgba(0,229,153,0.06)] text-[#f5f7f6]">
              {loading ? (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-[#8c9994]">
                ...
                filteredSeries.map((s) => (
                  <tr key={s.id} className="hover:bg-[#161f1c]/60 transition">
                    <td className="p-4 font-mono font-bold text-[#00e599]">#{s.id}</td>
                    <td className="p-4 font-bold">
                      <Link href={`/series/${s.id}`} className="hover:text-[#00e599] transition">
                        {mediaTitle(s)}
                      </Link>
                    </td>
                    <td className="p-4 text-[#8c9994] line-clamp-1 max-w-xs">{s.description || '—'}</td>
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/admin/episodes?series=${s.id}`}
                          className="flex items-center gap-1 rounded-xl border border-[rgba(0,229,153,0.2)] bg-[#161f1c] px-3 py-1.5 text-xs font-bold text-[#00e599] hover:bg-[#00e599] hover:text-[#080a0a]"
                        >
                          Qismlar
                        </Link>
                        <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-md">
          <div className="relative w-full max-w-lg rounded-3xl border border-[rgba(0,229,153,0.3)] bg-[#101514] p-6 sm:p-8 shadow-2xl">
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-4">
              <div>
                <label className="mb-1 block text-xs font-bold text-[#f5f7f6]">Serial nomi</label>
                <input
                  type="text"
                  value={title}
