    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[#8c9994]">
            58 ta ovozli funksiya va WebSocket orqali Groq LLM agenti tomonidan bajarilgan amallar logi
          </p>
        </div>

        <div className="flex items-center gap-2 rounded-xl border border-[rgba(0,229,153,0.2)] bg-[#101514] px-3.5 py-1.5 text-xs font-bold text-[#00e599]">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[rgba(0,229,153,0.06)] text-[#f5f7f6]">
              {logs.map((log) => (
                <tr key={log.id} className="hover:bg-[#161f1c]/60 transition">
                  <td className="p-4 text-[#8c9994] whitespace-nowrap">{log.timestamp}</td>
                  <td className="p-4 font-bold text-[#f5f7f6] italic">&ldquo;{log.transcript}&rdquo;</td>
                  <td className="p-4 font-mono font-bold text-[#00e599]">{log.action}</td>
                  <td className="p-4">
                    <span className="rounded-md border border-[rgba(0,229,153,0.2)] bg-[rgba(0,229,153,0.08)] px-2 py-0.5 text-[10px] font-bold text-[#00e599]">
                      {log.layer}
                    </span>
                  </td>
                  <td className="p-4 font-mono text-xs text-[#8c9994]">{log.duration_ms} ms</td>
                  <td className="p-4 text-right">
                    <span className="inline-flex items-center gap-1 text-[#00e599] font-bold text-[11px]">
